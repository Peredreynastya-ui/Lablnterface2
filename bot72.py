...
def get_text_messages(message):
    if message.text == "Привет":
        bot.send_message(message.chat.id, "Привет, это персональный чат-бот художницы Ольги Ромуальдовны Ионайтис, который поможет вам познакомиться с её творчеством. Вы можете подписаться на социальные сети, купить книги с её иллюстрациями или написать ей что-нибудь.")
        
        # Клавиатура с Web App кнопкой
        keyboard_web = types.ReplyKeyboardMarkup(resize_keyboard=True)
        ...
        keyboard_web.add(web_app_button)
        ...
        keyboard_inline.add(button_about, button_social, button_books)
        bot.send_message(message.chat.id, "Выбери один из вариантов", reply_markup=keyboard_inline)
